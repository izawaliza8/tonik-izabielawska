...
function SurfaceParticles({ baseDirs }: ParticlesProps) {
  return (
    <>
      {baseDirs.map((d, i) => (
        <mesh
          ...
          renderOrder={2}
        >
          <sphereGeometry args={[PARTICLE_SIZE, PARTICLE_SEGS, PARTICLE_SEGS]} />
          <meshBasicMaterial
...
function GlobeScene({
  ...
  return (
    <group ref={groupRef} scale={GLOBE_SCALE * INIT_SCALE}>
      ...
      <SurfaceParticles baseDirs={baseDirs} />
...
export default function GlobeV2({ className }: GlobeProps) {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      // Launch yoyo when beam P2→3 finishes drawing
      tl.call(() => {
