...
const DEBUG_SCROLL_MARKERS = process.env.NODE_ENV !== 'production'

function stMarkers(indent: number) {
  return DEBUG_SCROLL_MARKERS
    ? { startColor: '#4ade80', endColor: '#f87171', fontSize: '11px', indent }
    : false
}
...
function SurfaceParticles({ baseDirs, particleRefs, revealRef }: ParticlesProps) {
  ...
  const { bgColorA, bgColorB } = useControls('Dot background', {
    bgColorA: { value: '#905868', label: 'gradient A' },
    bgColorB: { value: '#210a11', label: 'gradient B' },
  })
  const bgColorARef = useRef(bgColorA)
  const bgColorBRef = useRef(bgColorB)
  bgColorARef.current = bgColorA
  bgColorBRef.current = bgColorB
  ...
  const bgMaterials = useMemo(() =>
    textures.map(() => new THREE.ShaderMaterial({
      uniforms: {
        uReveal:   { value: 0.0 },
        uBgColorA: { value: new THREE.Vector3(...hexToSRGB('#905868')) },
        uBgColorB: { value: new THREE.Vector3(...hexToSRGB('#210a11')) },
      },
  ...
  useFrame(() => {
    ...
    bgMaterials.forEach(m => {
      ...
      m.uniforms.uBgColorA.value.set(...hexToSRGB(bgColorARef.current))
      m.uniforms.uBgColorB.value.set(...hexToSRGB(bgColorBRef.current))
    })
    materials.forEach(m => {
...
function GlobeScene({
  ...
  const backLightPosRef = useRef(new THREE.Vector3(0, 0.15, -2.5))
  const backLightColorRef = useRef('#de91bb')
  const backLightMaxRef = useRef(12)
  const backLightRimRef = useRef(6)

  const {
    backLightX, backLightY, backLightZ,
    backLightColor, backLightMax, backLightRim, backLightPreview,
  } = useControls('Back light', {
    backLightX:       { value: 0,    min: -5, max: 5, step: 0.05, label: 'X' },
    backLightY:       { value: 0.15, min: -5, max: 5, step: 0.05, label: 'Y' },
    backLightZ:       { value: -2.5, min: -5, max: 5, step: 0.05, label: 'Z' },
    backLightColor:   { value: '#de91bb', label: 'color' },
    backLightMax:     { value: 12, min: 0, max: 16, step: 0.1, label: 'max strength' },
    backLightRim:     { value: 6, min: 0.5, max: 8, step: 0.1, label: 'rim power' },
    backLightPreview: { value: false, label: 'force on' },
  })

  // ── Phase-4 text controls (each block is its own Decal on the sphere surface) ──
  const { headerPx, headerColor, headerSize, headerPosY } = useControls('Header', {
    headerPx:    { value: 71,   min: 30,   max: 200, step: 1,    label: 'font px' },
    headerColor: { value: '#FFFFFF', label: 'color' },
    headerSize:  { value: 1.6,  min: 0.5,  max: 3.0, step: 0.05, label: 'decal size' },
    headerPosY:  { value: 0.36, min: -0.8, max: 0.8, step: 0.01, label: 'band Y' },
  })
  const { subtitlePx, subtitleColor, subtitleSize, subtitlePosY } = useControls('Subtitle', {
    subtitlePx:    { value: 47,   min: 16,   max: 120, step: 1,    label: 'font px' },
    subtitleColor: { value: '#BBBBBB', label: 'color' },
    subtitleSize:  { value: 0.95, min: 0.5,  max: 3.0, step: 0.05, label: 'decal size' },
    subtitlePosY:  { value: 0.18, min: -0.8, max: 0.8, step: 0.01, label: 'band Y' },
  })
  const { textLift, textZPush, showWireframe } = useControls('Layout', {
    textLift:      { value: 1.04, min: 1.0, max: 1.3, step: 0.01, label: 'lift off surface' },
    textZPush:     { value: -10,  min: -40, max: 0,   step: 1,    label: 'polygon offset' },
    showWireframe: { value: false, label: 'Globe wireframe' },
  })
  const { textDepth, textDepthAngle, textSideColor } = useControls('Text depth', {
    textDepth:      { value: 10,  min: 0, max: 60,  step: 1, label: 'thickness px' },
    textDepthAngle: { value: 90,  min: 0, max: 360, step: 1, label: 'angle°' },
    textSideColor:  { value: '#1c1c1c', label: 'side color' },
  })
  ...
  useFrame(({ camera }, delta) => {
    ...
    backLightPosRef.current.set(backLightX, backLightY, backLightZ)
    backLightColorRef.current = backLightColor
    backLightMaxRef.current = backLightMax
    backLightRimRef.current = backLightRim

    u.uBackLight.value.copy(backLightPosRef.current)
    u.uBackLightStrength.value = backLightPreview
      ? backLightMax
      : backLightStrengthRef.current
    ...
    const yoyoActive = yoyoStartedRef.current && p3Raw < PHASE1_REST_EPS && sig <= 0.5
  ...
  return (
    <>
      ...
      <group ref={groupRef} scale={GLOBE_SCALE * INIT_SCALE}>
        ...
        {showWireframe && (
          <mesh renderOrder={2} scale={1.002}>
            <icosahedronGeometry args={[1, detail]} />
            <meshBasicMaterial wireframe color="#555555" transparent opacity={0.3} depthWrite={false} />
          </mesh>
        )}
      </group>
      ...
      <PhaseFourText
        ...
        depth={textDepth}
        depthAngle={textDepthAngle}
        sideColor={textSideColor}
      />
